 ...
 */
export function deriveKeyFromPath(seed: Uint8Array, walletIndex: number): Uint8Array {
 ...
 */
function deriveChild(parentKey: Uint8Array, chainCode: Uint8Array, index: number): { key: Uint8Array; chainCode: Uint8Array } {
 ...
 */
export async function createKeypairFromMnemonic(mnemonic: string, walletIndex: number = 0): Promise<Keypair> {
...
export function getNextAccountIndex(existingWallets: any[], masterMnemonic: string): number {
  ...
  const relatedWallets = existingWallets.filter(wallet => 
    ...
    (wallet.derivationPath && wallet.derivationPath.startsWith("m/44'/501'/0'/"))
  );
