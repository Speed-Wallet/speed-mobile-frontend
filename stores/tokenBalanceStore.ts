...
async function fetchNativeSolBalance(
...
export const useTokenBalanceStore = create<TokenBalanceStoreState>(
  (set, get) => ({
    ...
    _initializeBalances: () => {
      ...
      allTokens.forEach((token) => {
        initialTokenBalanceDetails[token.address] = {
          // Updated variable
          ...token,
          balance: 0,
          rawBalance: BigInt(0),
      ...
      set({
        tokenBalanceDetails: initialTokenBalanceDetails,
        storeError: null,
        wsError: null,
      }); // Renamed property
    },
    ...
    _fetchInitialBalances: async (walletPublicKey: PublicKey) => {
      ...
      set((state) => {
        ...
        allTokens.forEach((token) => {
          ...
          if (newTokenBalanceDetails[token.address]) {
            // Updated variable
            ...
            newTokenBalanceDetails[token.address] = {
              // Updated variable
        ...
        return {
          tokenBalanceDetails: newTokenBalanceDetails,
          storeError: null,
        }; // Renamed property
      });
      ...
      const balancePromises = allTokens.map(async (token) => {
        try {
          const result =
            token.address === WSOL_MINT
              ? await fetchNativeSolBalance(walletPublicKey)
              : await fetchSplTokenBalance(token, walletPublicKey);

          set((state) => {
            const newTokenBalanceDetails = { ...state.tokenBalanceDetails }; // Renamed variable and property
            if (newTokenBalanceDetails[result.address]) {
              // Updated variable
              newTokenBalanceDetails[result.address] = {
                // Updated variable
    ...
    _connectWebSocket: (walletPublicKey: PublicKey) => {
      ...
      newWs.onopen = () => {
        ...
        console.log(
          'WebSocket connection opened for token balance subscriptions (Zustand).',
        );
        set({
          ws: newWs,
          wsError: null,
        ...
        }); // Clear errors and reset attempts on successful connection
        ...
        set({
          requestIdToMintAddress: currentRequestIdToMintAddress,
          rpcSubIdToMintAddress: {},
        });
      };
      ...
      newWs.onmessage = (event) => {
        ...
        if (
          message.result &&
          message.id &&
          state.requestIdToMintAddress[message.id]
        ) {
          // Subscription confirmation
          const mintAddress = state.requestIdToMintAddress[message.id];
          set((s) => ({
            rpcSubIdToMintAddress: {
              ...s.rpcSubIdToMintAddress,
              [message.result]: mintAddress,
            },
            requestIdToMintAddress: (({ [message.id]: _, ...rest }) => rest)(
              s.requestIdToMintAddress,
            ), // Remove handled ID
          }));
          ...
          if (
            ...
            if (accountData?.parsed?.info?.tokenAmount) {
              humanBalance = parseFloat(
                accountData.parsed.info.tokenAmount.uiAmountString || '0',
              );
              rawAmount = BigInt(
                accountData.parsed.info.tokenAmount.amount || '0',
              );
            }
            ...
            // This part might need adjustment based on how SOL is treated (is it in `allTokens` with a special address?)
              ...
              message.params.result.value.lamports !== undefined &&
              tokenInfo.address === WSOL_MINT
            ) {
              // Assuming SOL is a token in your list and its balance comes from lamports
              humanBalance =
                message.params.result.value.lamports / 10 ** tokenInfo.decimals;
            ...
            tokenInfo &&
            message.params.result &&
            message.params.result.value === null
          ) {
            // Account closed
          ...
          message.id.startsWith('heartbeat_')
        ) {
      ...
      newWs.onclose = (event) => {
        ...
        console.log(
          `WebSocket connection closed after ${connectionDuration}ms. Code: ${event.code}, Reason: ${event.reason || 'No reason provided'}`,
        );
        get()._clearHeartbeat(); // Clear heartbeat on close

        if (get().ws === newWs) {
          // Only update if this is the active WebSocket instance
          set((state) => ({
            ws: null,
            isConnectingOrFetching: false,
            connectionStartTime: null,
          ...
          if (event.code !== 1000 && get().activeWalletPublicKey) {
            if (connectionDuration < 30000) {
              console.log(
                'Connection was short-lived, indicating possible server issues',
              );
            }
    ...
    _scheduleReconnect: (walletPublicKey: PublicKey) => {
      ...
      if (state.reconnectAttempts >= maxAttempts) {
        ...
        set({
          wsError: new Error('Failed to reconnect after multiple attempts'),
        });
        return;
      }
    ...
    subscribeToTokenBalances: async (walletPublicKeyStr: string | null) => {
      ...
      if (
        state.isConnectingOrFetching &&
        state.activeWalletPublicKey === walletPublicKeyStr
      ) {
      ...
      if (state.activeWalletPublicKey !== walletPublicKeyStr) {
        ...
        set({
          activeWalletPublicKey: walletPublicKeyStr,
          tokenBalanceDetails: {},
          storeError: null,
          wsError: null,
          reconnectAttempts: 0,
